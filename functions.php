...
function uploadFile($target_dir, $fileToUpload, $extensions, &$fileName)
{
    ...
    // VÈrifier que cette extension est acceptable
    if (! in_array($imageFileType, $upperExtensions)) {
        $uploadOk = false;
    }
    
    // TODO: TRES IMPORTANT : Il reste ‡ vÈrifier si le fichier est une image ou non
    
    // VÈrifier la taille du fichier
    if ($_FILES[$fileToUpload]["size"] > 4096) {
        
        $uploadOk = false;
    }
    
    // Si y a pas de problËmes
    if ($uploadOk) {
        
        // DÈplacer le fichier vers son emplacement sur le serveur
...
<body style="background-color:#E6E6FA;">
 ...
 <table summary="Tableau de r&eacute;sultat">
  ...
  else
  {
    ...
    echo "<tr>";
    // importation des variables √† partir de l'√©tiquette des champs
    extract($une_personne,EXTR_OVERWRITE);
    echo'<tr><td><img  width="50" height="50" ; src="'  . $photo. '"></td><td>'.$nom.'</td><td>'.$prenom.'</td><td>'.$telephone1.
    '</td><td>'.$telephone2. '</td><td>'.$adresse.'</td><td>'.$emailPersonnel.
    '</td><td>'.$emailProfessionnel.'</td><td>'.$genre.'</td></tr>';
...
/**
 * Neutralise la chaine de caractËre envoyÈe par POST
 */
...
/**
 * Neutralise la chaine de caractËre envoyÈe par GET
 */
...
function Recherche_sur_critere($crit_recherche,$val_recherche)
{
    ...
    switch($crit_recherche)
    {
        ...
        case 'telephone1':$val_recherche="%".normalisation_numerique($val_recherche)."%";break;
        case 'telephone2':$val_recherche="%".normalisation_numerique($val_recherche)."%";break;
    ...
    try
    {
        if (!empty($val_recherche) && $val_recherche!="%%")
        {
            ...
            switch($crit_recherche)
            {
                case 'nom'    : $RequetePreparee->bindParam(':val_recherche', $val_recherche, PDO::PARAM_STR, 50);break;
                case 'telephone1': case 'telephone2' : $RequetePreparee->bindParam(':val_recherche', $val_recherche, PDO::PARAM_STR, 50);break;
            }
            ...
            {
                extract($une_personne,EXTR_OVERWRITE);
                $tab_personnes_retourne[$nom]=$une_personne;
            }
...
{
    // tableau des caract√®res accentu√©s √† remplacer
    $caracteres_a_remplacer     = array( );
    // tableau des caract√®res sans accent de remplacement
...
function printCheckBox($key, $lab,$lab1, $msg = '')
{
    ...
    <input  name="contact[]"   class="form-check-input" type="checkbox" value="' . $key . '">
...
/**
 * Affiche un groupe de check box avec possiblitÈ d'affiche d'un message d'erreur de validation
 *
 */
...
function getCommunities()
{
    ...
    try {
        ...
        $bd = connexionDB();
        // Enregistrer les donnÈes dans la base de donnÈes
